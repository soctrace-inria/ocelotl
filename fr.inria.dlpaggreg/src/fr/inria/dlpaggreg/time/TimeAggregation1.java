...
public class TimeAggregation1 extends TimeAggregation {
	...
	public TimeAggregation1(List<Double> values) {
		super();
		this.values = values;
		setSize(values.size());
	}
	...
	protected void computeQualities() {
		List<List<Double>> sumValues = new ArrayList<List<Double>>();
		List<List<Double>> entValues = new ArrayList<List<Double>>();
		...
		for (int j=1; j<size; j++){
			for (int i=0; i<size-j; i++){
				...
				qualities.get(i).get(j).setGain
				(Complexity.entropyReduction(sumValues.get(i).get(j), entValues.get(i).get(j)));
				qualities.get(i).get(j).setLoss
				(Complexity.divergence(j+1, sumValues.get(i).get(j), entValues.get(i).get(j)));
			}
		}
	}
