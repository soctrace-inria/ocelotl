...
public class TimeAggregation3 extends TimeAggregation {
	...
	public TimeAggregation3(final List<List<List<Double>>> values) {
		super();
		this.values = values;
		setSize(values.size());
	}
	...
	protected void computeQualities() {
		final List<List<List<List<Double>>>> sumValues = new ArrayList<List<List<List<Double>>>>();
		final List<List<List<List<Double>>>> entValues = new ArrayList<List<List<List<Double>>>>();
		qualities = new ArrayList<List<DLPQuality>>();
		final int size2 = values.get(0).size();
		final int size3 = values.get(0).get(0).size();
		...
		for (int i = 0; i < size; i++)
			for (int k = 0; k < size2; k++)
				for (int l = 0; l < size3; l++) {
					...
					entValues.get(i).get(0).get(k).set(l, Complexity.entropyReduction(values.get(i).get(k).get(l), 0.0));
				}
			...
			for (int i = 0; i < size - j; i++)
				for (int k = 0; k < size2; k++)
					for (int l = 0; l < size3; l++) {
						...
						qualities.get(i).get(j).addToGain(Complexity.entropyReduction(sumValues.get(i).get(j).get(k).get(l), entValues.get(i).get(j).get(k).get(l)));
						qualities.get(i).get(j).addToLoss(Complexity.divergence(j + 1, sumValues.get(i).get(j).get(k).get(l), entValues.get(i).get(j).get(k).get(l)));
					}
	}
	...
	public void setValues(final List<List<List<Double>>> values) {
		this.values = values;
		setSize(values.size());
	}
