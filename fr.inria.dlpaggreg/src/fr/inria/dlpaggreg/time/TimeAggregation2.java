...
public class TimeAggregation2 extends TimeAggregation {
	...
	public TimeAggregation2(final List<List<Double>> values) {
		super();
		this.values = values;
		setSize(values.size());
	}
	...
	protected void computeQualities() {
		final List<List<List<Double>>> sumValues = new ArrayList<List<List<Double>>>();
		final List<List<List<Double>>> entValues = new ArrayList<List<List<Double>>>();
		...
		for (int j = 1; j < size; j++)
			for (int i = 0; i < size - j; i++)
				for (int k = 0; k < size2; k++) {
					...
					qualities.get(i).get(j).addToGain(Complexity.entropyReduction(sumValues.get(i).get(j).get(k), entValues.get(i).get(j).get(k)));
					qualities.get(i).get(j).addToLoss(Complexity.divergence(j + 1, sumValues.get(i).get(j).get(k), entValues.get(i).get(j).get(k)));
				}
	}
	...
	public void setValues(final List<List<Double>> values) {
		this.values = values;
		setSize(values.size());
	}
